...
The adversary is the AI client itself; see [AI-facing privacy](../../GLOSSARY.md#ai-facing-privacy) for the threat axes. The boundary is AI <-> [privacy gateway](../../GLOSSARY.md#privacy-gateway) <-> LND ([Node A](../../GLOSSARY.md#node-a), Voltage-hosted, Mutinynet/signet) - a boundary that exists only when the operator enables the advanced Lightning extension (`SPACER_MODE=lightning|full|ecash`; ecash mode enables this surface unchanged, doc 07 §9). The default onchain arbiter refuses `query_channels` / `manage_lightning` uniformly at its mode gate (audit `decision_refuse_mode`) and never imports the LND client, so none of this surface is reachable there; the per-call policies below apply as written whenever the extension is on. World-facing threats (LN gossip, peers, explorers, hosting) are out of scope - see the archived world-facing doc.
...
| Call | Returns | Sev | Mitigation |
|---|---|---|---|
| `sendcoins` | txid; publishes our UTXOs + change address on-chain | HIGH | gate destination through the [recipient address registry](../../GLOSSARY.md#recipient-address-registry) (operator-owned-internal endpoints only - internal fund management, never an external payment; §7); coin-select inside the arbiter |
...
- **2026-06-28 (external-value reframe, doc 12):** the BTC/LN write ops are **internal fund management only**. `manage_bitcoin` / `manage_lightning` (and `openchannel`) target only operator-owned endpoints; the [recipient address registry](../../GLOSSARY.md#recipient-address-registry) is recast as the **operator-owned-internal allowlist** (its external-destination role is gone), so an external recipient simply has no token. Reason: any external payee that can see a UTXO is inside the adversary's observability set, so an external on-chain payment would break UTXO privacy by construction. **All external value movement is eCash-only.** This names the guarantee the §3 mitigations jointly provide - **endpoint privacy:** the proxy never exposes the operator's source UTXOs, change, channel topology, or funding txids to the PET. The recast registry holds operator **output descriptors / xpubs** (not raw addresses): each internal send derives a fresh unused address (BIP32 HD, no reuse) the AI never sees, every derived address is operator-verifiable and provably the operator's (no-redirect), and registering a descriptor is console-only - so both whitelisting and per-send address generation are AI-free (full mechanism in the GLOSSARY registry entry + doc 05 §4.7). The op names are unchanged; the semantics are internal-only (no rename). External private LN is a deferred future option, not current. Full statement + implication map: `12--2026-06-27-1337-external-value-boundary-and-scale-tradeoff.md`.
