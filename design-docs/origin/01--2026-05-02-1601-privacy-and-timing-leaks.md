...
- **2026-06-28 (external-value reframe, doc 12):** the BTC/LN write ops are **internal fund management only** - `manage_bitcoin` / `manage_lightning` (renamed from `send_*`; the code rename landed as sp-3mm) target only operator-owned endpoints through the recast [recipient address registry](../../GLOSSARY.md#recipient-address-registry) (the operator-owned-internal allowlist; the descriptor / fresh-address mechanism is the GLOSSARY entry + doc 05 §4.7), and **all external value movement is eCash-only** - an external recipient simply has no token. This names the guardrail the §3 mitigations jointly provide: **endpoint privacy** - the PET never sees source UTXOs, change, channel topology, or funding txids. External private LN is a deferred future option, not current. Full statement + implication map: `12--2026-06-27-1337-external-value-boundary-and-scale-tradeoff.md`.
