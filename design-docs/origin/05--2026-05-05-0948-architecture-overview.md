...
**Date:** 2026-05-05 (reconciled 2026-06-12)
...
**Related:**
- `2026-05-02-1601-privacy-and-timing-leaks.md` / `2026-05-02-1603-bitcoind-privacy-and-timing-leaks.md` - per-API leak surfaces.
- `07--2026-06-12-0916-ecash-extension.md` - the eCash extension: custody split, mint threat model, delay scaling.
- `~/spacer/archive/2026-05-02-1700-node-privacy-from-the-world.md` - world-facing privacy (archived; separate threat model).
...
**Status: partial.** Test-mode windows are wired end-to-end (5-15s action, 5-15s result, 1-5s rejection-delivery; §10). Production is blocked behind the dynamic-window calculation (§7) and raises `NotImplementedError` on every enqueue path until it lands - the safe failure mode is "does not run," not "runs with the wrong window." Mode selection is conservative: only exact `SPACER_TIMING_MODE=test` enables test mode (a typo, miscase, or unset lands in production and refuses). The `enqueue` / `due` / `pending` APIs and both tables exist; the executor that drains due actions against bitcoind / LND and the consumer that drains due results are not yet wired.
...
(Storage is still SQLite-backed; the YAML migration is the next milestone, bl-2lbqu4.)
...
**2026-06-12, eCash build (sp-2hwco4.2):** reconciled again, against `ecash.py` and the extended `gateway.py` / `timing.py` / `standing_approvals.py` / petcli; §§1-6 and §10 fold in the eCash extension as landed - the three-mode ladder with `full` frozen at onchain + lightning, the allowance-before-approvals gate, the structural `mint` destination, the `_petcli_local` envelope, §4.9, and the no-ecash-import gate. Doc 07 §11 is the detailed eCash reconcile. Still deferred (forward-looking design, not yet wired):

- **Registry YAML migration** (bl-2lbqu4) - storage is still SQLite-backed; §4.7's storage-substrate description is the target.
- **Production timing** (action / result / rejection windows, now also the eCash mint-boundary gaps) - blocked on §7's dynamic-window calculation (per-rail under the delay-scaling principle, doc 07 §7; the mint-activity source is doc 07 §10.1); `NotImplementedError`-gated.
...
- **Timing-layer executor / result-registry consumer** - the queues and APIs exist, but the drainers that run due actions against bitcoind / LND / the mint (fund/defund orchestration, doc 07 §3) and surface due results are not yet wired into the gateway write path (this gates the §10 happy-path send variants and the `ecash_funding_rate` rate cap, doc 07 §8).
- **State-defaults split** - `state.py` still defaults to `arbiter/data/state.db` while `audit.py` moved to `arbiter/state/`; both fold into the broader arb-auditability tree migration.
- **Live mint environment** (sp-2hwco4.4) - the assumed nutshell CLI surface (§4.9) is unverified until a real test mint stands; documented as its own NN doc when it does.
