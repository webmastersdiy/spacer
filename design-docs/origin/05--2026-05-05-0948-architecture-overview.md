...
**Internal-management vs external-value (the external-value reframe).** Across all modes, the BTC/LN writes - `manage_bitcoin`, `manage_lightning` - are **internal fund management only**: they target only operator-owned endpoints in the [recipient registry](#47-recipient-address-registry) (recast as the operator-owned-internal allowlist), and never pay an external recipient. The reason is endpoint privacy - any external payee that can see a UTXO is inside the adversary's observability set, so an external on-chain payment would break UTXO privacy by construction. **All external value movement is eCash-only** (the bearer float the petitioner spends at the mint; doc 07). So `onchain` and `lightning` are **internal-management** modes and `ecash` is the **sole external-value** mode - which is also why amount-scale privacy holds with eCash off and loosens (to the float's scale only) with eCash on. The full statement and implication map are the external-value-boundary doc (`12--2026-06-27-1337-external-value-boundary-and-scale-tradeoff.md`); external private LN is a deferred future option, not current.

**Naming (`manage_*`, not `send_*`).** These ops are named `manage_bitcoin` / `manage_lightning` (not `send_*`) for three reasons: it makes the internal-only nature explicit at the name; the name agrees with the enforced rail (they may target only operator-owned endpoints, the recast registry); and it keeps "send" / concerning wording **calibrated** - naming routine internal moves `send_*` would either alarm the operator (it reads like an external payment) or, worse, desensitize them so a genuinely concerning op gets ignored, whereas `manage_*` reserves alarming language for what warrants it. The code rename **landed as sp-3mm**: gateway routing, petcli commands (`submit manage-bitcoin`, `advanced manage-lightning`), and the exit-loop tree all use `manage_*`, so docs and code read the same.
...
Per request, in order: [pseudonymize](../../GLOSSARY.md#pseudonymize-identifier-pseudonymization) (which for writes doubles as the destination gate, resolving `recipient_token` through the [recipient address registry](#47-recipient-address-registry) and refusing uniformly on miss; the eCash writes skip this step - their destination is structurally the pinned mint), an [eCash allowance](../../GLOSSARY.md#ecash-allowance) cap on `fund_ecash` ahead of any approval (ecash mode; §2.2, doc 07 §8), a [standing approvals](../../GLOSSARY.md#standing-approvals) check (writes only; eCash writes match the literal destination `mint`; no match parks in [HITL](../../GLOSSARY.md#human-in-the-loop-hitl-approval)), [band](../../GLOSSARY.md#banding-numeric-value-banding) cloak-ineligible numeric fields, serve read ops from the [read snapshot](../../GLOSSARY.md#read-snapshot-snapshot-served-reads) (doc 15: the [scale-cloak](../../GLOSSARY.md#scale-cloaking) - **in place of** banding, the per-tier scale already compresses 10x+ - and the serve-grid quantization ran at refresh time on the executor sweep's clock; before the first refresh the gateway refuses uniformly rather than touch a backend), refuse extension-gated ops whose extension the mode does not enable (§2.2; no HITL park - the operator already decided by choosing the mode), [HITL](../../GLOSSARY.md#human-in-the-loop-hitl-approval)-defer unrecognized ops, [audit log](../../GLOSSARY.md#audit-log) append, and [latency-normalize](../../GLOSSARY.md#latency-normalization) every response to a configured floor so timing carries no information about which step refused or how far the pipeline got.

**Implementation status.** The `arbiter/src/gateway.py` skeleton wires the HTTP entry, JSON parsing, audit logging, latency normalization, mode routing (§2.2; lazy LND and eCash imports, `decision_refuse_mode` on extension-gated ops), registry-gated write resolution, the eCash write branch (allowance gate then standing approvals, `decision_refuse_allowance` on the cap; §2.2, doc 07 §8), the standing-approvals gate (both branches: HITL park on no-match, dispatch on match), the HITL park for unrecognized ops, the result-poll fast path (§4.8), and snapshot-served reads (doc 15, as of sp-ksx: `_dispatch` serves the per-op `read_snapshots` row verbatim and refuses uniformly when none exists; the executor drainer's sweep refreshes it - backend read, `present()`, serve-grid quantization - on a randomized event-independent clock, bitcoind-backed in onchain mode, LND-backed under the extensions, `query_channels` extension-only). As of sp-uwa0v0 (2026-06-18) an approved write - all four named ops, fund/defund included - enqueues on the timing layer and `executor.py` drains it against the daemons / the mint; the `not_implemented` tail in `_dispatch` is reachable only by an unknown op. As of sp-tb0 a *refused* state-changing call defers rather than refusing synchronously: every write-op gate miss (denomination, registry, standing approval, the eCash allowance cap, the extension/mode gate for a disabled write, and the unknown-op HITL park) routes through `_defer_rejection_and_ack`, which returns the same `received` + handle a pass returns and enqueues a `kind="rejection"` on the timing layer; the refusal surfaces only on the petitioner's later poll (§4.7) as a uniform payload identical to a dispatched-but-failed send, so submit leaks no pass-vs-refuse signal and no which-gate signal. Malformed input and a not-yet-refreshed read still refuse synchronously (neither is a state-changing gate decision). Still pending: band-edge randomization and aggregate-by-default on cloak-ineligible fields, the HITL queue table behind `_hitl_park` (currently audit-logs the deferral but writes no queue row; the refusal itself is now deferred through the result registry, not returned synchronously), and production timing / cloak / snapshot-refresh windows (§4.6). Each pending mechanism is a named function (`_hitl_park`, `_band_outbound`), so a reviewer sees wired behavior vs present structure at a glance.

### 4.2 Bitcoin client access (primary surface)

The onchain (default) mode's backend: `bitcoin-cli` against the [local bitcoind](../../GLOSSARY.md#local-bitcoind) under `spacer/arbiter/bitcoin/`, via `arbiter/src/bitcoin.py` (stdlib subprocess, no shell, four RPCs; full detail in `02--2026-05-02-1602-bitcoind-mutinynet-test-flow.md` §6). `query_balance` is fed from here - the snapshot refresh sweep (doc 15, §4.1) runs `getbalance()`, a BTC Decimal scaled to integer sats, on the drainer clock; the request path serves the stored row - and `manage_bitcoin` dispatches through `sendtoaddress` via the §4.6 executor. Coin selection and signing stay inside bitcoind; PSBTs never leave ([hide secrets](../../GLOSSARY.md#hide-secrets)).
...
- Balance / channel-capacity reads (`walletbalance`, `channelbalance`) feed the [read snapshot](../../GLOSSARY.md#read-snapshot-snapshot-served-reads)'s refresh sweep (doc 15), [scale-cloaked](../../GLOSSARY.md#scale-cloaking) into a fixed 0-100k window at refresh time **in place of** [banding](../../GLOSSARY.md#banding-numeric-value-banding) (the per-tier scale already gives 10x+ compression); the request path serves the stored value. Banding stays for non-cloak fields (per-call fee / amount on send paths).
...
The internal-endpoint gate: a state-changing BTC/LN call carries a `recipient_token`, the gateway resolves it through `registry.lookup()`, and any non-`ok` outcome collapses to the uniform "destination unavailable" refusal - the lookup *is* the gate, with no separate outbound-policy step. Post the external-value reframe (modes para above) the registry is the **operator-owned-internal endpoint allowlist**: every resolvable token names one of the operator's own wallets/nodes, so the gate confines `manage_bitcoin` / `manage_lightning` to internal fund management, and an external recipient simply has no token (external value is eCash-only). The standard treatment (storage YAML, entry shape, one-time-use, probing infeasibility, refusal behavior) is the glossary [Recipient address registry](../../GLOSSARY.md#recipient-address-registry) entry. The implementation specifics beyond it:

- **Descriptor entries + fresh-address derivation** (the recast registry; full spec in the GLOSSARY [Recipient address registry](../../GLOSSARY.md#recipient-address-registry) entry): entries are operator-owned **output descriptors / xpubs** (xpub + script type + derivation path), **not raw addresses**. Each internal `manage_bitcoin` derives the next unused address (BIP32 HD, tracking a next-index + gap-limit) so no address is reused; the AI never sees the raw address, and every derived address is operator-verifiable against the descriptor (no-redirect, since only the operator holds the keys). Registering a descriptor is console-only, no AI.
...
- **Derived-address encodings:** the descriptor's script type fixes the derived-address encoding - bech32 (P2WPKH/P2WSH), bech32m (P2TR), or base58check (P2PKH/P2SH) - and the arbiter derives in that encoding. Post-reframe the gate no longer parses operator-supplied raw addresses (entries are descriptors); the internal LN target is the operator's own node issuing a fresh BOLT-11/12 invoice per transfer, so the earlier "LN-side encodings / Lightning Address are open (§7)" question is moot (no external LN destinations to encode).
...
- [Standing approvals](../../GLOSSARY.md#standing-approvals): gateway, after registry resolution (or, for the eCash writes, after the allowance gate - their destination is structurally `mint`, doc 07 §3), gating *which* writes dispatch without a [HITL](../../GLOSSARY.md#human-in-the-loop-hitl-approval) pause. Empty by default, so every write HITLs until the operator ratifies patterns. Writes only; reads are snapshot-served with no approval gate (doc 15). (Wired; both branches exercised in the exit loop, §10.)
...
- [Read snapshot](../../GLOSSARY.md#read-snapshot-snapshot-served-reads): gateway read path (`query_balance` / `query_channels`) serves per-op snapshots refreshed on an event-independent randomized clock by the executor sweep, localizing any balance/capacity change to one refresh epoch instead of its actual moment. **Status: wired** (sp-ksx; exit loop §10) - test-mode 5-15s band; production epochs gated with §7's dynamic window.
...
**Execution:** validations are state-independent and parallelize in principle, but the current runner (`test-harness/scripts/exit_loop_runner.py`) is sequential - 46 variants plus the no-lnd-import / no-ecash-import / mint-contract gates (49 checks), ~25s per pass, each spinning a fresh in-thread arbiter on an ephemeral port (per-variant `SPACER_MODE` across all three modes; unset = onchain default) and running `petcli` as a subprocess. The loop is binary with no manual sign-off: a failure's raw output is captured under `exit-loop/`, fixed, and re-run until every variant passes.

**Artifacts** live under `exit-loop/petcli/<command>/<variant>/` (`stdout.log`, `stderr.log`, `arbiter-events.log`, `infra-events.log`, `result.json`), mirroring the petcli tree - Lightning artifacts sit under `petcli/advanced/`; the runner rebuilds the tree from its manifest each pass. In the current pass `infra-events.log` is a marker file: read variants stage their snapshot via the runner's refresh precondition, whose sweep reads `bitcoin.py` against a runner-installed fake `bitcoin-cli` in onchain mode (canned replies via `$BITCOIN_CLI_SCENARIO`) or `lnd.py` against a fake `lncli` under the extensions (`$LNCLI_SCENARIO`) - the petcli read itself never touches a backend; the eCash local wallet variants shell through a fake petitioner-side `cashu` (`$CASHU_SCENARIO` at `$PETCLI_CASHU_BIN`), and write variants stop at the mode / allowance / registry / standing-approvals gates or assert the timing-layer acknowledgment against the fakes - no manifest variant reaches a real daemon (only `--live` does). Deliberately, no fake exists for the arbiter-side cashu wrapper: no manifest variant can reach it, and unset `CASHU_BIN` / `CASHU_MINT_URL` make an unexpected arbiter-side mint call error loudly instead of being absorbed (doc 07 §9). No variant exercises live bitcoind / LND / mint yet.
