...
**Date:** 2026-05-05 (reconciled 2026-05-24)
...
[Spacer](../../GLOSSARY.md#spacer) lets an AI client drive a Bitcoin and Lightning node without learning more about the operator's wallet, balances, or identifiers than the task requires. Two processes joined by a protocol; the protocol is the trust boundary. Out of scope here (and listed in §8): [world-facing privacy](../../GLOSSARY.md#world-facing-privacy), wire formats, API shapes, data-model schemas, code structure, and the bitcoin/LN primitives (all in the glossary or their own per-decision docs).
...
- The [arbiter](../../GLOSSARY.md#arbiter): server-side, unreachable by the AI; bundles the [privacy gateway](../../GLOSSARY.md#privacy-gateway), the bitcoin/LND client access, local state, the [audit log](../../GLOSSARY.md#audit-log), and the timing layer ([action delay](../../GLOSSARY.md#action-delay) / [result delay](../../GLOSSARY.md#result-delay)).
...
- **No transient files outside the gitignored subtrees**: every module writes only under `state/` / `data/` / `bitcoin/` / `lnd/` (gitignored per `06--2026-05-24-0623-arb-auditability.md` §3). Churn inside `src/` / `config/` / `bin/` would pollute the [continuous-snapshot](06--2026-05-24-0623-arb-auditability.md) history the manual audit primitive depends on.
...
Per request, in order: [pseudonymize](../../GLOSSARY.md#pseudonymize-identifier-pseudonymization) (which for writes doubles as the destination gate, resolving `recipient_token` through the [recipient address registry](#47-recipient-address-registry) and refusing uniformly on miss), a [standing approvals](../../GLOSSARY.md#standing-approvals) check (writes only; no match parks in [HITL](../../GLOSSARY.md#human-in-the-loop-hitl-approval)), [band](../../GLOSSARY.md#banding-numeric-value-banding) cloak-ineligible numeric fields, [scale-cloak](../../GLOSSARY.md#scale-cloaking) wallet/channel totals (**in place of** banding - the per-tier scale already compresses 10x+), [HITL](../../GLOSSARY.md#human-in-the-loop-hitl-approval)-defer unrecognized ops, [audit log](../../GLOSSARY.md#audit-log) append, and [latency-normalize](../../GLOSSARY.md#latency-normalization) every response to a configured floor so timing carries no information about which step refused or how far the pipeline got.

**Implementation status.** The `arbiter/src/gateway.py` skeleton wires the HTTP entry, JSON parsing, audit logging, latency normalization, registry-gated write resolution, the HITL park for unrecognized ops, the result-poll fast path (§4.8), and read-op dispatch (`query_balance`, `query_channels`) through scale cloaking. Pending: the standing-approvals check, the timing layer + bitcoind / LND executor (§4.6), band-edge randomization and aggregate-by-default on cloak-ineligible fields, and the HITL queue table behind `_hitl_park` (currently audit-logs the deferral and returns the refusal but writes no queue row). Each pending mechanism is a named function (`_hitl_park`, `_band_outbound`, the write-dispatch fallthrough in `_dispatch`), so a reviewer sees wired behavior vs present structure at a glance.

### 4.2 Bitcoin client access

`bitcoin-cli` against the [local bitcoind](../../GLOSSARY.md#local-bitcoind) under `spacer/arbiter/bitcoin/`, via `arbiter/src/bitcoin.py` (stdlib subprocess, no shell, four RPCs; full detail in `02--2026-05-02-1602-bitcoind-mutinynet-test-flow.md` §6). Coin selection and signing stay inside bitcoind; PSBTs never leave ([hide secrets](../../GLOSSARY.md#hide-secrets)).

### 4.3 LND client access

`lncli` via a subprocess argv list (no shell) in `arbiter/src/lnd.py` - deliberately **not** the test-harness [lncliA](../../GLOSSARY.md#lnclia) shell script, so a reviewer (§2.1) reads one `_run` helper and sees the whole connection surface (binary, TLS cert, macaroon, RPC server, network). `lncli` already speaks gRPC, so "lncli plus gRPC/REST" collapses to "lncli only" (a direct gRPC client remains a future swap). Reconciled behavior:
...
- [Standing approvals](../../GLOSSARY.md#standing-approvals): gateway, after registry resolution, gating *which* writes dispatch without a [HITL](../../GLOSSARY.md#human-in-the-loop-hitl-approval) pause. Empty by default, so every write HITLs until the operator ratifies patterns. Writes only; reads dispatch unconditionally. (Not yet wired - bl-gfxsxx.)
...
- [HITL](../../GLOSSARY.md#human-in-the-loop-hitl-approval): gateway parks a write matching no standing approval, or any unrecognized op (outside reads `query_balance` / `query_channels` and writes `send_bitcoin` / `send_lightning`). Assent arrives out-of-band at the arbiter console; anything crossing back to the AI is hand-retyped (no clipboard). A recognized write with a `recipient_token` miss refuses at the registry gate, not HITL.
...
- [Default --private channels](../../GLOSSARY.md#default---private-channels): LND client path on channel-open (§4.3).
...
- [Action delay](../../GLOSSARY.md#action-delay) / [result delay](../../GLOSSARY.md#result-delay): the timing layer (§4.6), per-action across hours-to-days; they subsume per-poll cadence (the arbiter <-> daemon link is inside the trust boundary). **Status: partial** - test-mode wired; production gated behind §7's dynamic window.
...
**Current coverage:** read-only `query/balance`, `query/channels`, `result/poll`, and `estimate/window` are exercised end-to-end, including the T0 no-cloak branch and the four cloaked `scale.present()` branches (T1 init; T2 init = identical wire response to T1, which is the cloak's point; transition-pending with `drift > range`; transition-applied, which audit-logs `scale_tier_shift_applied`). State-changing sends are exercised only on the registry-miss refusal path; happy-path sends and the per-cause rejection subcases (expired, used, bad checksum, anomalous) are blocked behind the timing-layer executor wire-up in `gateway.py:_dispatch` (write ops currently return `{"status": "not_implemented"}`) and land alongside it.
...
**Execution:** validations are state-independent and parallelize in principle (target: the `.beads/` queue via the Gas City rigs), but the current runner (`test-harness/scripts/exit_loop_runner.py`) is sequential - 17 variants, ~12s per pass, each spinning a fresh in-thread arbiter on an ephemeral port and running `petcli` as a subprocess. The loop is binary with no manual sign-off: a failure's raw output is captured under `exit-loop/`, fixed, and re-run until every variant passes.

**Artifacts** live under `exit-loop/petcli/<command>/<variant>/` (`stdout.log`, `stderr.log`, `arbiter-events.log`, `infra-events.log`, `result.json`); the runner rebuilds the tree from its manifest each pass. In the current pass `infra-events.log` is a marker file: read variants dispatch through `lnd.py` against a runner-installed fake `lncli` (canned replies via `$LNCLI_SCENARIO`), write variants refuse at the registry gate without reaching `lnd.py` / `bitcoin.py`, and no variant exercises live bitcoind / LND yet.

---

## 11. Reconciliation status

Reconciled 2026-05-24 against `gateway.py`, `lnd.py`, `bitcoin.py`, `audit.py`, `scale.py`, `timing.py`, `results.py`, and the petitioner shim; the bodies of §4-§5 and §10 reflect the implementation as landed (including the token format, no-mainnet rule, audit-log durability, the cloak-replaces-banding decision, and the transport-error envelope). Still deferred (forward-looking design, not yet wired):

- **Standing-approvals check** in `gateway.py` (bl-gfxsxx, blocked on bl-hu56z9). The §3 / §4.1 / §6 entries are accurate forward design.
- **Registry YAML migration** (bl-2lbqu4) - storage is still SQLite-backed; §4.7's storage-substrate description is the target.
- **Production timing** (action / result / rejection windows) - blocked on §7's dynamic-window calculation; `NotImplementedError`-gated.
...
- **Timing-layer executor / result-registry consumer** - the queues and APIs exist, but the drainers that run due actions against bitcoind / LND and surface due results are not yet wired into the gateway write path (this gates the §10 happy-path send variants).
